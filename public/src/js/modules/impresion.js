...
const Impresion = {
    ...
    renderResumenOT: function(orden) {
        ...
        // Cargar y mostrar acumulado de produccion de turnos anteriores
    ...
    // ==================== ACUMULADO DE OT (turnos anteriores) ====================
    ...
    cargarAcumuladoOT: async function(orden) {
        ...
        if (typeof AxonesDB !== 'undefined' && AxonesDB.isReady()) {
            try {
                const { data } = await AxonesDB.client.from('produccion_impresion')
                    .select('*').eq('numero_ot', numOT).order('created_at', { ascending: false });
                registros = data || [];
            } catch (e) { console.warn('[Impresion] Error cargando acumulado:', e); }
        }
        ...
        // Color de la barra segun porcentaje
        ...
        <div id="acumuladoOTPanel" class="card border-info mb-3" style="border-width: 2px;">
            ...
            <div class="card-body py-2">
                <div class="row g-2 mb-2">
                    <div class="col-md-3">
                        <small class="text-muted d-block">Pedido Total</small>
                        <strong style="font-size:1.1rem;">${pedidoKg.toLocaleString('es-VE')} Kg</strong>
                    </div>
                    <div class="col-md-3">
                        <small class="text-muted d-block">Producido</small>
                        <strong style="font-size:1.1rem;color:#198754;">${totalPesoSalida.toFixed(1)} Kg</strong>
                    </div>
                    <div class="col-md-3">
                        <small class="text-muted d-block">Falta por Producir</small>
                        <strong style="font-size:1.1rem;color:${faltante > 0 ? '#dc3545' : '#198754'};">${faltante.toFixed(1)} Kg</strong>
                    </div>
                    <div class="col-md-3">
                        <small class="text-muted d-block">Registros / Turnos</small>
                        <strong style="font-size:1.1rem;">${registros.length}</strong>
                    </div>
                </div>
                <div class="progress mb-2" style="height:24px;">
                    <div class="progress-bar ${barColor}" role="progressbar" style="width:${porcentaje}%; font-weight:bold;">
        ...
        // Insertar antes del formulario de produccion
        const form = document.getElementById('formImpresion');
